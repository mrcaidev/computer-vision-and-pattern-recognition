...
__all__ = ["create_results_webpage"]
...
def create_results_webpage(
    ...
    files = glob.glob("results/thumbnails/*.jpg")
    for f in files:
        os.remove(f)

    if not os.path.isdir("results"):
        os.mkdir("results")
    if not os.path.isdir("results/thumbnails"):
        os.mkdir("results/thumbnails")
    ...
    with open("results/index.html", "w+") as f:
        ...
        print(f"Accuracy (mean of diagonal of confusion matrix): {accuracy:.3%}")
